...
Verbatim port of the Hermes provider's client.py; mirrors the
invariants of @everme/agent-sdk client.js:
...
The client is synchronous on purpose (urllib) — the async backend wraps
calls in asyncio.to_thread so we carry zero third-party dependencies
...
class EverMeClient:
    ...
    def _headers(self) -> Dict[str, str]:
        return {
            ...
            "User-Agent": f"everme-raven-plugin/{self._version} (agentId={self._agent_id})",
        }
