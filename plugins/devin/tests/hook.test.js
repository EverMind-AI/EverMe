...
const fixture = path.join(packageDir, "tests", "fixtures", "devin-transcript.jsonl");
const tempDirs = [];
const servers = [];
...
describe("everme-devin hook CLI", () => {
  test("stores the latest conversational turn without code-action bodies", async () => {
    const requests = [];
    const server = await startServer(async (req, res) => {
      requests.push({ path: req.url, body: JSON.parse(await readBody(req)) });
      respond(res, 200, { status: 0, result: { flushed: false } });
    });
    ...
    const result = await runHookProcess({
      ...
      tool_info: { transcript_path: fixture },
    }, runtime.env);

    assert.equal(result.code, 0);
    assert.equal(result.stdout, "");
    assert.match(result.stderr, /saveAgentMemory ok: .*requestId=[0-9a-f-]{36}/,
      "stderr must carry the save line with its trace id");
    assert.equal(requests.length, 1);
    assert.equal(requests[0].path, "/api/v1/mem/agent-memory");
    assert.equal(requests[0].body.conversationId, "devin-trajectory");
    assert.equal(requests[0].body.flush, false);
    assert.deepEqual(
      requests[0].body.messages.map(({ role, content }) => ({ role, content })),
      [
    ...
    assert.doesNotMatch(JSON.stringify(requests[0].body), /print\('hello world'\)/);
  });

  test("backend errors fail open and redact the credential", async () => {
    const token = ["ev", "t_", "e".repeat(32)].join("");
    ...
    const result = await runHookProcess({
      agent_action_name: "post_cascade_response_with_transcript",
      trajectory_id: "devin-failed",
      execution_id: "devin-failed-execution",
      tool_info: { transcript_path: fixture },
    }, runtime.env);
...
function runHookProcess(input, env) {
